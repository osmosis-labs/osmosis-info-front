    ...
    return (
        <div className={classes.selects}>
            <div className={classes.selectContainer}>
                ...
                <Image className={`${classes.image} ${classes.select}`} assets={true}
                    src={`${selectedTokens.one?.symbol?.toLowerCase()}.png`}
                    pathAssets='../assets/'
                    srcFallback="default.png" alt={`${selectedTokens.one.symbol}`} />
                <Select
                ...
                >
                    {tokens.map(token => {
                        return <MenuItem disabled={selectPairs.two === token.denom} key={`one-${token.denom}`}
                            value={token.denom}>{token.symbol.length > 0 ? token.symbol : token.denom}</MenuItem>
                    })}
                </Select>
            </div>
            ...
            <div className={classes.selectContainer}>
                <Image className={`${classes.image} ${classes.select}`} assets={true}
                    ...
                    pathAssets='../assets/'
                    srcFallback="default.png" alt={`${selectedTokens.two.symbol}`} />
                <Select
                    className={classes.select}
                ...
                >
                    {tokens.map(token => {
                        return <MenuItem disabled={selectPairs.one === token.denom} key={`two-${token.denom}`}
                            value={token.denom}>{token.symbol.length > 0 ? token.symbol : token.denom}</MenuItem>
                    })}
                </Select>
            </div>
