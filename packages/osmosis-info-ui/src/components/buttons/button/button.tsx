...
export const Button = ({
	...
	if (variant === "secondary") {
		if (disabled) {
			classNameDefaut += `${classNameDefaut} border-2 border-wosmongton-200  opacity-50 cursor-not-allowed py-3.5`;
		} else {
			classNameDefaut += `${classNameDefaut} border-2 border-wosmongton-400 hover:border-wosmongton-200 transition-all py-3.5`;
		}
	} else {
		if (disabled) classNameDefaut += `${classNameDefaut} opacity-50 bg-osmoverse-500 cursor-not-allowed `;
