...
const getOrphanFormatted = ({ key, translation }: { key: string; translation: string }): null | StringFormatted => {
	const startTag = key;
	const regEx = new RegExp(`${startTag}`);
	const matchTag = translation.match(regEx);
...
const getStringFormatted = ({ key, translation }: { key: string; translation: string }): null | StringFormatted => {
	const startTag = key;
	const endTagReg = key[0] + "/" + key.slice(1); // add "/" for closed tag
	const regEx = new RegExp(`${startTag}.*${endTagReg}`);
	const componentTextTranslatedReg = translation.match(regEx);
	// Check if component is in translation
	if (componentTextTranslatedReg && componentTextTranslatedReg.length > 0 && componentTextTranslatedReg.index) {
		// clear translation by removing the first and last tag
		let componentTextTranslated = componentTextTranslatedReg[0].replace(startTag, "");
...
const getFormattedComponent = ({
	...
	if (key.includes("/")) {
		const orphanFormatted = getOrphanFormatted({ key, translation });
	...
	if (typeof component.props.children === "string" || component.props.children === undefined) {
		// Get formatted string
		const stringFormatted = getStringFormatted({ key, translation });
		...
		if (Array.isArray(component.props.children)) {
			children = component.props.children.map((child: ReactElement, index: number) => {
...
export const Formatted = ({ translationKey, values, components }: FormattedProps) => {
	...
	if (componentsFormatted && componentsFormatted.length > 0) {
		...
		const body: Array<ReactElement> = [];
		let currentIndex = 0;
		// replace tags by formatted children
		componentsFormatted.forEach((componentFormatted, index) => {
			body.push(
				<React.Fragment key={index}>{translation.slice(currentIndex, componentFormatted.startIndex)}</React.Fragment>
			);
			body.push(componentFormatted.element);
			currentIndex = componentFormatted.startIndex + componentFormatted.length;
		});
		// add the end of body
		body.push(<React.Fragment key={"endbody"}>{translation.slice(currentIndex)}</React.Fragment>);
		return <>{body}</>;
	}
	return <>{translation}</>;
};
